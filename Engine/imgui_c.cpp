...
void imgui_indent(void) { ImGui::Indent(); }
...
bool imgui_slider_float(const char* label, f32* v, f32 v_min, f32 v_max) { return ImGui::SliderFloat(label, v, v_min, v_max); }
bool imgui_slider_int(const char* label, i32* v, i32 v_min, i32 v_max) { return ImGui::SliderInt(label, v, v_min, v_max); }
bool imgui_color_edit3(const char* label, f32 v[3]) { return ImGui::ColorEdit3(label, v, ImGuiColorEditFlags_NoInputs); }
bool imgui_color_edit4(const char* label, f32 v[4]) { return ImGui::ColorEdit4(label, v, ImGuiColorEditFlags_NoInputs); }
bool imgui_input_int(const char* label, i32* v) { return ImGui::InputInt(label, v); }
bool imgui_input_float(const char* label, f32* v) { return ImGui::InputFloat(label, v); }
bool imgui_combo(const char* label, i32* current_idx, const char* const* items, i32 items_count) { return ImGui::Combo(label, current_idx, items, items_count); }
bool imgui_selectable(const char* label, bool selected) { return ImGui::Selectable(label, selected); }
bool imgui_collapsing_header(const char* label) { return ImGui::CollapsingHeader(label); }
bool imgui_radio(const char* label, i32* v, i32 v_button) { return ImGui::RadioButton(label, v, v_button); }
...
bool imgui_is_item_hovered(void) { return ImGui::IsItemHovered(); }
bool imgui_is_window_hovered(void) { return ImGui::IsWindowHovered(); }
bool imgui_is_any_item_hovered(void) { return ImGui::IsAnyItemHovered(); }
...
void imgui_open_popup(const char* str_id) { ImGui::OpenPopup(str_id); }
bool imgui_begin_popup(const char* str_id) { return ImGui::BeginPopup(str_id); }
void imgui_end_popup(void) { ImGui::EndPopup(); }
void imgui_close_current_popup(void) { ImGui::CloseCurrentPopup(); }
void imgui_push_id_int(i32 id) { ImGui::PushID(id); }
void imgui_pop_id(void) { ImGui::PopID(); }
f32 imgui_get_font_size(void) { return ImGui::GetFontSize(); }
f32 imgui_calc_text_width(const char* text) { return ImGui::CalcTextSize(text).x; }
